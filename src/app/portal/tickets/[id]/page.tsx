...
export default async function CustomerTicketPage({ 
  ...
  const copy = portalCopy(facing.locale);
  const dateLocale = facing.locale === 'en' ? 'en-GB' : 'de-DE';
  ...
  if (!ticket) {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-64 text-center">
        <h2 className="text-2xl font-bold mb-2">{copy.ticketMissingTitle}</h2>
        <p className="text-slate-500">{copy.ticketMissingBody}</p>
        <Link href="/portal" className="mt-6 text-primary hover:underline">{copy.ticketBack}</Link>
      </div>
    );
  }
  ...
  const formattedTicket = {
    ...
    createdAt: new Intl.DateTimeFormat(dateLocale, { dateStyle: 'medium', timeStyle: 'short' }).format(ticket.createdAt),
    workspace: ticket.workspace.name,
    messages: ticket.messages.map((m) => ({
      id: m.id,
      author: m.authorId === session.email ? copy.ticketMe : copy.ticketSupport,
      type: m.authorId === session.email ? 'customer' : 'agent',
      body: m.body,
      time: new Intl.DateTimeFormat(dateLocale, { timeStyle: 'short' }).format(m.createdAt)
    }))
  };
      ...
      <div className="flex flex-col gap-10">
        <div className="flex items-center justify-between">
          <Link href="/portal" className="group flex items-center gap-3 text-on-surface-variant/60 hover:text-slate-900 dark:hover:text-white transition-all">
            ...
            <span className="text-xs font-bold tracking-widest uppercase">{copy.ticketBack}</span>
          </Link>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex flex-col items-start md:items-end gap-2 text-xs font-medium text-slate-500 dark:text-white/40">
             <div className="flex items-center gap-2">
               ...
               <span>{copy.ticketCreated} {formattedTicket.createdAt}</span>
             </div>
             <div className="flex items-center gap-2">
               <MessageSquare className="w-3.5 h-3.5" />
               <span>{formattedTicket.messages.length} {copy.ticketMessages}</span>
             </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 group">
        <div className="relative p-1 bg-gradient-to-b from-slate-200 to-transparent dark:from-white/10 dark:to-transparent rounded-[42px] transition-all focus-within:from-slate-400 dark:focus-within:from-white/30">
          <div className="bg-white dark:bg-surface-container rounded-[40px] p-10 flex flex-col gap-8 shadow-2xl dark:shadow-none">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold text-on-background dark:text-white">{copy.ticketCompose}</h2>
            ...
            <div className="relative">
              <textarea 
                placeholder={copy.ticketComposePlaceholder}
            ...
            <div className="flex items-center justify-between">
               <p className="text-[11px] text-slate-400 dark:text-white/20 font-medium whitespace-nowrap overflow-hidden text-ellipsis mr-4">{copy.ticketAttachLater}</p>
